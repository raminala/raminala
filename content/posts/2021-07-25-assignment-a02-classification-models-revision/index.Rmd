...
Above plot and table give an impression of the age of examined group. The minimum, maximum and average ages are 19, 87, and 41 years respectively. The majority of observations are concentrated at the age of between 30 to 35 years. This information would not provide any insight into the possible outcome.
...
It shows small positive correlation between age and balance. generally older clients tends to have higher balance but the correlation is more like a random relation  because it is more close to zero (random case) than 1 (strong correlation). It shows that these two parameters cannot be assessed from each others. On the other hand, highly correlated variables are not good predictors together because they follow a same trend.
...
In this plot we take a look at the distribution of subscription_status by age of clients. Plots shows age has a relatively low effect on the chance of subscription of a client. This will be proved later by multiple linear regression and looking at coefficient of age and its p-value. Despite age has low effect, older clients has slightly hight chance of subscription. This can be seen by comparing mean and third quartile lines.
...
from Generalized Linear Model coefficient, it appears that education is a determinant factor, therefore an analysis of education and subscription status performed here. This plot shows that most clients have secondary education, followed by tertiary, primary and unknown education level. Comparing the ratio of people who subscribed to those did not subscribe in each categories shows that people with tertiary education is more likely to finally subscribe.

## Evaluating performance with confusion matrix

It is important to assess accuracy of model. Although model should be assesses with test data (not previously seen data or data that is not used for building the model), but In this part we will use model train data for assessment. We pass data to both single and multiple Logistic regression Models in order to classify whether clients associated with the given information in the train data set will subscribe to a term deposit. besides, we will evaluate the performance of the model by confusion matrix and heatmap plot.
...
First, start with evaluating the performance of single logistic regression Model using heatmap matrix.
...
Here, We set threshold to 0.2. After passing the data to SLR model,it shows 88.5% of predictions are accurate. There are 521 false negative and 2 false positive predictions.
