...
For this dataset, the LOOCV method takes long time to process. Because it consists building the model for (4521-1) times, evaluating the remaining sample with the model and averaging error. We are using tictoc library to measure the running time for each code chunk to evaluate performance of these methods in terms of running time.
...
#LOOCV_run_time= end_time - start_time
...
the execution time for LOOCV is vaey large.