...
bank %>%
  ...
  geom_point()
```
This plot consists of two numerical variables (age and balance) that both are explanatory variables and response variable which depicted by color (pink colors did not subscribed and blue colours subscribed). 
...
Finally, some parameters that seemed to be determinant in univariate analysis are not important in fact. This is because of corrolation between non determinant and determinant 
variables.
...
- Univariate analysis  of job shows that a big postion of students, unemployed, and those with unknown jobs subscribed, but respondents with blue-color jobs and technicians did not subscribe so often.
