...
In the above sections two models achieved: ***Single linear regression (SLR)*** and ***Multiple linear regression (MLR)***. In the following paragraphs three models will calculate using resampling: LOOCV, K-fold, and Bootstrap. So, we will have five models in total. performance of them will be compared.
...
**Disadvantage:** The execution time could be so long if n (number of observation) is large. Fortunately, we have 450 observations in *diamond* dataset and my machine with 16GB of RAM easily executed this. Besides, we know that low bias means high variance and the method is more prone to overfitting.

Other disadvantage of this method is it tests the model performance against *one data point* at each iteration.This might result to higher variation in the prediction error, especially if some data points are outliers.
...
The obvious advantage of k-fold over LOOCV is K-fold is faster (needs less computation)). A less obvious but potentially more important advantage of k-fold CV is that it often gives more accurate estimates of the test error rate than does LOOCV.

Trade-off:

***low K:*** is more biased (undesirable).

***high K:*** large variance (undesirable).



here, we consider *k=5* for 440 observations.
...
ggplot(test_results_SLR, aes(price ,Price_pred))+
  ...
  coord_obs_pred() +
  labs(x = 'Actual diamond Price', y = 'Prediction by SLR')+
...
ggplot(test_results_MLR, aes(price ,Price_pred))+
  ...
  geom_abline(color = 'blue', linetype = 2) +
  coord_obs_pred() +
  labs(x = 'Actual diamond Price', y = 'Prediction by MLR')+
...
ggplot(test_results_model_LOOCV, aes(price ,Price_pred))+
  ...
  geom_abline(color = 'blue', linetype = 2) +
  coord_obs_pred() +
  labs(x = 'Actual diamond Price', y = 'Prediction by LOOCV')+
...
ggplot(test_results_model_K_fold, aes(price ,Price_pred))+
  ...
  geom_abline(color = 'blue', linetype = 2) +
  coord_obs_pred() +
  labs(x = 'Actual diamond Price', y = 'Prediction by K-fold')+
  annotate('text', x = 800, y = 4000, label = 'R^2=0.9670232',  size = 5, color="red")+
  xlim(0, 4000)+ylim(0, 4000)
...
ggplot(test_results_model_Bootstrap, aes(price ,Price_pred))+
  ...
  geom_abline(color = 'blue', linetype = 2) +
  coord_obs_pred() +
  labs(x = 'Actual diamond Price', y = 'Prediction by Bootstrap')+
  annotate('text', x = 800, y = 4000, label = 'R^2=0.9623576',  size = 5, color="red")+
  xlim(0, 4000)+ylim(0, 4000)
